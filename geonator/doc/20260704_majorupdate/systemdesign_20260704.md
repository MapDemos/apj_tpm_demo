...
| `adjacent` | 目の前・すぐ隣・すぐ横 | **80m**（circle固定） | —（常にcircle） |
| `very_close` | すぐ近く・出てすぐ | **200m** | 約3分 |
| `nearby` | 近く・付近・そば | **500m** | 約6分 |
| `somewhat_nearby` | 少し歩く | **900m** | 約11分 |
| `far` | かなり遠い | 距離条件として使わない（→ pushback） | — |

（閾値は寛容に設定：POI点は建物重心なので「目の前」でも点間距離が30〜50m出る。スコアリング（§5-4）が近い候補を上位（金）に並べるため、閾値を広く取っても薄まらない。ユーザーが「徒歩5分」「200m以内」等の具体数値を入力した場合はテーブルより優先。）
