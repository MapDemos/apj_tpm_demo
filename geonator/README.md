...
> 本READMEは **現在のソースコードのみ** を根拠に記述しています（外部情報は参照していません）。設計の確定版ドキュメントは `doc/20260704_majorupdate/` を参照。
...
├── data/poi-blocklist.js POI名ブロックリスト（ノイズ除去）
├── styles/main.css       スタイル（チャット壁紙・候補パネル・ティア色・レスポンシブ）
├── images/               チャット背景画像
├── doc/                  設計ドキュメント（旧 readme/。仕様の正）
...
- 旧・LLM主導（エージェントループ）版は撤去済み。現行は JS主導版のみ。設計の確定版は `doc/20260704_majorupdate/`（`systemdesign` / `flowdetail` / `implementation_instructions`）。
