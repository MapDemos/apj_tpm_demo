...
**ブランド/チェーン指定 と カテゴリ＋属性/下位種別 の区別（重要）**:
- 意図が **特定ブランド/チェーン**（例:ローソン、スターバックス、ゴールドジム）→ 同カテゴリの**別ブランドは no**（意図はその店を指すため。ローソン→ファミマ=no）。
- 意図が **カテゴリ＋属性/下位種別**（例:クロスフィットジム、女性専用ジム、24時間営業のカフェ、個室サウナ）→ その**属性・下位種別は名前に出ないことが多い**ので、**基盤カテゴリ（ジム/カフェ/サウナ）が一致する候補は原則 unknown（残す）**。名前が属性を明示していれば definitely/probably。no にするのは**基盤カテゴリ自体が違うもの**（例:クロスフィットジム→歯科/寺/駅）に限る。一般ジムを「クロスフィットではない」と決めつけて no にして**全滅させない**（属性の有無は現地確認事項）。
