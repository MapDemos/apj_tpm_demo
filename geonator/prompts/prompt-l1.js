...
- **target が曖昧な時だけ target.alternatives を使う**：文法構造の無いキーワード羅列で、探す対象が2つ以上の候補に割れて確定できない場合（例:「新丸子駅、マンション、ローソン、徒歩3分以内」＝マンションを探すのかローソンを探すのか不明）、最も可能性が高いものを target.text にしつつ、他の妥当な**探索対象候補**を target.alternatives（文字列配列）に入れる。システムがユーザーに「何をお探しですか？」と確認する。
  - **文法的に対象が明確なら alternatives は必ず空**（[]）。距離修飾が付いた目印（「ローソンが目の前」「〜から3分」）は明らかに condition なので alternatives に入れない。誤って毎回出さないこと。
