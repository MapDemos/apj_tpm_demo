...
**「そこ」「ここ」「あそこ」「それ」等の指示語だけでは場所の手がかりにならない**（具体的な地名・駅名・施設名がテキスト中に無い限り、指示語が何を指すか分からない）。この場合も proximity.anchors は絶対に埋めず not_a_query を返すこと。「現在地」「この辺」等のプレースホルダーを作って anchors を埋めるのは禁止（存在しない情報の捏造になる）。
（例：「そこが13階建てなの」「それを探してよ」「探して〜」→ 具体的な地名が無いので not_a_query）

## ノイズの多い文章からの抽出（重要）
現実の入力は雑談・前置き・感情・体験談・言い訳・脱線が混ざる。**場所探しに無関係な部分は捨て、地理的な意図（target / proximity / conditions）だけを抽出する**こと。
- **長く散らかった文章でも、場所を探す意図が読み取れれば必ずスキーマを返す**（ノイズが多いだけで not_a_query にしない）。手がかりが全く無い時だけ not_a_query。
- 無関係な語を conditions や unsupported_features に**入れない**（例:「お腹すいた」「疲れた」「昨日〜と話してて」「まあいいや」等は完全に無視して捨てる）。
...
- 例:「あー疲れた。そういえば昨日さ、新丸子のドミノピザあるじゃん？あの隣のマンション気になってて、9階建てだった気がする。まあどうでもいいんだけど」
  → target=マンション(floors:9), proximity/scope=新丸子, conditions=[{poi:ドミノピザ, 隣=very_close}]。「疲れた」「どうでもいい」等は捨てる。
...
### 必ず出す（常に含める）
- \`proximity.anchors[]\`：各 \`{ "type", "text" }\`。type = station | poi | address | locality | intersection
- \`target\`：\`{ "text", "query_intent" }\`。query_intent = category_mansion | category_apartment | category_building | specific | category_busstop | category_busstop_location | intersection | signal | transit_entrance（**"specific" でも必ず出す**）
- \`confirmation\`：ユーザー向けの確認文（下記ルール）

### 値がある時だけ出す（無い/既定なら省略）
- \`proximity.anchors[].specificity\`：\`"generic"\` の時だけ（既定 specific）
- \`proximity.anchors[].subtype\`：出入口指定がある時だけ \`{ "exit": "A4" }\` 等
- \`proximity.scope\`：行政区域での絞り込みがある時だけ \`{ "type", "text" }\`
- \`proximity.bearing_filter\`：方角指定がある時だけ north | south | east | west
- \`proximity.within\`：基準点からの明示的な距離/時間がある時だけ（下記ルール）
- \`target.queries\`：text 以外の検索語を足す QE 展開をする時だけ（text と同一なら省略＝JSが [text] を補完）
- \`target.floors\`：階数指定がある時だけ。**必ず比較の向きを区別する**：完全一致「N階建て」→ \`{ "value": N }\`／「N階建て**以下**・N階**未満**・N階**まで**」→ \`{ "max": N }\`／「N階建て**以上**・N階**超**・N階**より上**」→ \`{ "min": N }\`（タワマン/超高層/高層→ \`{ "min": 20 }\`、低層/平屋/背の低い→ \`{ "max": 3 }\`）。反転「N階建てではない」→ \`{ "value": N, "negate": true }\`。**「以下」を \`value\` にしない（＝ちょうどN階に潰さない）**
- \`target.category_tag\`：下記「category_tag」ルール参照
- \`conditions[]\`：近接条件がある時だけ。各条件は最小で \`{ "type", "text" }\`。type = poi | road | water | rail | intersection | signal | transit_entrance | category_busstop
  - \`query_intent\`：category_busstop 等の時だけ（既定 specific）
  - \`queries\`：QE 展開をする時だけ（poi のみ・下記QEルール）
  - \`direction\`：方向指定がある時だけ
  - \`negate\`：「〜が無い/入っていない/ではない」の時だけ \`true\`
  - \`distance\`：距離/近接度の指定がある時だけ。radius は \`{ "level": "same_building|adjacent|roadside|very_close|nearby|somewhat_nearby|far" }\`、時間指定は \`{ "method": "isochrone", "minutes": N, "profile": "walking|cycling|driving" }\`、実距離は \`{ "meters": N }\`。**無指定なら distance 自体を省略**（JSが既定レベルを補完）
  - \`category_tag\`：type=poi の時だけ・下記「category_tag」ルール参照
- \`unsupported_features\`：数値化・地図化できない非地理的特徴がある時だけ（文字列配列・下記ルール）
- \`result_area\`：「だいたいこの辺」の曖昧依頼の時だけ \`true\`
- \`interpretations\`：構造が曖昧で複数解釈を提示する時だけ（下記ルール）

### 最小出力の例
入力「西大島駅の近くのマンション、すぐ近くにローソン」→
\`\`\`json
{
  "proximity": { "anchors": [{ "type": "station", "text": "西大島駅" }] },
  "target": { "text": "マンション", "query_intent": "category_mansion" },
  "conditions": [{ "type": "poi", "text": "ローソン", "distance": { "level": "very_close" } }],
  "confirmation": "西大島駅の近くで、すぐ近くにローソンがあるマンションですね。探しますので少々お待ちください。"
}
\`\`\`
（scope / bearing_filter / within / floors / negate / direction / specificity / queries / result_area / unsupported_features / interpretations は指定が無いので**書かない**）
...
### interpretations（解釈が割れる時だけ・最大3）
入力の**構造**（探す対象＝target / 目印＝proximity / 条件＝condition の役割、および距離の帰属）が
**2通り以上に妥当に読める**時だけ、各解釈を表す**完結した日本語の一文**を interpretations 配列に入れる（最大3・先頭＝最有力＝上のスキーマと一致）。システムがJSでボタン提示し「どの意図か」を選ばせる。
- **原則ここは空（[]）にする**。1つに読めるなら interpretations は出さない（毎回出さない）。
- **発動する例**：「新丸子駅、マンション、ローソン、徒歩3分以内」（マンションを探す?ローソンを探す? 3分は駅から?ローソンから?）
  → interpretations=[
     "新丸子駅から徒歩3分以内の、ローソンが近くにあるマンション",
     "ローソンから徒歩3分以内の、新丸子駅エリアのマンション",
     "新丸子駅の近くの、マンションの近くにあるローソン"]（最大3・尤度順）
- **発動する例②**：「新丸子駅徒歩1分以内のカフェとローソンとファミマ」（一般カテゴリの「カフェ」とチェーン店「ローソン」「ファミマ」が「と」で並列され、文末の名詞句が複数の異なる種類のPOIをまとめている。どれが探す対象でどれが条件か構造上決まらない）
  → interpretations=[
     "新丸子駅から徒歩1分以内の、ローソンとファミリーマートが近くにあるカフェ",
     "新丸子駅から徒歩1分以内にある、カフェかローソンかファミリーマートのいずれか"]（最大3・尤度順）
  ※ このように**一般カテゴリ＋チェーン店が「と」で並列**され、それが**文末の名詞句全体**を構成している場合は、後述の「チェーン店は原則condition」というヒューリスティックだけで機械的に1つのtargetへ畳み込まず、interpretationsで確認する。
- **発動しない例**：「新丸子駅の近くの、ローソンが目の前にあるマンション」（役割が文法で確定）／「渋谷駅の近くのカフェ」（対象1つ）／「新丸子駅から徒歩3分のマンション」（帰属明確）→ interpretations=[]。
- **判定の指針**：物件系(マンション/家/ビル)が1つなら target 確定・チェーン店(ローソン/スタバ等)は原則 condition・「〜が目の前/〜から○分/〜の近く」等の距離修飾が付いた名詞は condition 確定 → これらで役割が決まるなら interpretations は空。**構造マーカーの無い名詞の羅列**で初めて割れる（文末が複数の異なるPOI種別を「と」で並べただけの名詞句である場合も、これに該当し割れる＝発動する例②）。
- ※ 地名の曖昧（青山→南青山、池袋どの市）は interpretations では扱わない（システムが別途「もしかして/どちら」で確認する）。ここは**構造**の曖昧だけ。
...
- 形式: { "profile": "walking|cycling|driving|null", "profile_inferred": false, "minMinutes": 数値|null, "maxMinutes": 数値|null, "maxMeters": 数値|null, "level": "adjacent|very_close|null" }
- **時間（isochrone）**（対応する移動手段は 徒歩=walking / 自転車=cycling / 車=driving の**3つだけ**）:
  ...
  - **②移動手段の指定が無い「n分」**（例:「〜から5分以内」）→ profile:"walking" を仮定し **profile_inferred:true**（JSがユーザーへ「徒歩と解釈した」旨を通知する）
  - **③電車・バス・公共交通・タクシー等の所要時間**（例:「電車で5分」「バスで10分」）→ **対応不能**。isochroneにせず（within側は設定しない）、原文を **unsupported_features** に入れる（徒歩/自転車/車に**読み替えない**）
...
- distance.method の判定（重要）:
  ...
  - **②移動手段の指定が無い「x分」** → profile:"walking" を仮定し **profile_inferred:true**
  - **③電車・バス・公共交通・タクシー等の所要時間** → 対応不能。isochroneにせず（distanceに時間を入れず）、原文を **unsupported_features** に入れる（他の移動手段に読み替えない）
- distance.level の目安:
...
### category_tag（target・poi条件のみ・任意）
このプロンプトの後に「POI Category Taxonomy」としてcanonical_id一覧（JSON配列）が付与されている場合のみ使う。
- target（query_intent=specific）または condition.type=poi のとき、探している物が taxonomy 中の**1件と明確に一致**するなら、その文字列をそのまま \`category_tag\` に入れる（一般常識で対応が分かるケース含む。例:「牛丼屋」→「レストラン>丼もの」）。
- **固有ブランド名・店名（例:「ローソン」「ドミノピザ」「ファミリーマート」）はそれ自体がカテゴリではないため \`category_tag\` を出力しない（省略）**。名前検索で確実に見つかるので、無理にtaxonomyへ当てはめない。
- 複数のカテゴリに当てはまりそう、または該当が無い/確信が持てない場合も出力しない（無理に当てはめない）。
