...
- **人に通じる分かりやすいもの**を優先（公園・公共施設・有名チェーン・駅・大型店など）。無名・番地のみ・一般的すぎるものは避ける。
- 各提案は短い条件文にする。日本語なら「すぐ近くに◯◯がある」、英語なら「◯◯ is right nearby」。

出力はJSONのみ（前後に説明なし）。良い目印が無ければ空配列:
\`\`\`json
{"suggestions": ["すぐ近くに天神中央公園がある", "すぐ近くに福岡市役所がある"]}
\`\`\`
`;
