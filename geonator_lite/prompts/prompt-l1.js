...
- \`cond[]\`：近接条件がある時だけ。各条件は最小で \`{ "ty", "tx" }\`。ty = poi | road | water | rail | intersection | signal | transit_entrance | category_busstop
  ...
  - \`d\`：距離/近接度の指定がある時だけ。radiusは \`{ "lv": "same_building|adjacent|roadside|very_close|nearby|somewhat_nearby|far" }\`、時間指定は \`{ "m": "isochrone", "mi": N, "pf": "walking|cycling|driving" }\`、実距離は \`{ "me": N }\`。**無指定なら d 自体を省略**（JSが既定レベルを補完）
...
- 形式: { "pf": "walking|cycling|driving", "pfi": false, "mnMi": 数値, "mxMi": 数値, "mxMe": 数値, "lv": "same_building|adjacent|very_close" }（不要なキーは省略）
...
- **近接度(lv)**: 「〜の**目の前**の/**すぐ隣**の」→ { lv:"adjacent" }（目の前・真ん前・真横）/ { lv:"very_close" }（すぐ隣・すぐそば・ほぼ隣接）/ { lv:"same_building" }（「〜の中の」「〜内の」等、同じ建物内であることが明示されている場合）
...
- ng: **反転条件**。その条件物が「無い/入っていない/避けたい/離れている」ことを求めている場合に true。
  例:「同じ建物にファミマが入っていないマンション」→ {ty:poi, tx:"ファミマ", ng:true, d:{lv:same_building}}、
...
- **タクソノミーに一致するカテゴリが無く、実在の店名パターンも予測できない複合語（国名/地名+ショップ・店等）は、複数の推測複合語を作らず、その業態を特徴づける最短のキーワード1つだけを追加する**。Search Box自体が部分一致で検索するので、実際の店名が「〜マート」でも「〜センター」でも「〜物産店」でも、短いキーワードなら実在の綴りを推測しなくても拾える。「ベトナム物産店」「ベトナム食材店」のような推測複合語を複数作るのは誤り（実在の店名と一致する保証が無い）。
